...
function initializeFirebaseClient() {
  if (getApps().length === 0) {
    // In a deployed Firebase App Hosting environment, process.env.FIREBASE_CONFIG is automatically set.
    // We check for this environment variable to determine which config to use.
    const firebaseEnvConfig = process.env.FIREBASE_CONFIG;
    if (firebaseEnvConfig) {
        try {
            const config = typeof firebaseEnvConfig === 'string' ? JSON.parse(firebaseEnvConfig) : firebaseEnvConfig;
            firebaseApp = initializeApp(config);
        } catch (e) {
            console.error("Failed to parse FIREBASE_CONFIG or initialize app. Falling back to local config.", e);
            // If parsing fails, fall back to the local config file.
            firebaseApp = initializeApp(firebaseConfig);
        }
    } else {
        // Fallback for local development or other environments where FIREBASE_CONFIG is not set.
        firebaseApp = initializeApp(firebaseConfig);
    }
  } else {
    firebaseApp = getApp();
