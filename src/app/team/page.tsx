  ...
  return (
    <div className="container max-w-7xl mx-auto px-4 py-12 md:py-24">
      ...
      <div className="text-center mb-12">
        <h1 className="font-headline text-4xl md:text-5xl font-bold text-chart-5 uppercase italic tracking-tighter">Meet <span className="text-chart-1">The Team</span></h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {artists.map((artist) => (
          <Card key={artist.name} className="flex flex-col border-none shadow-xl bg-white rounded-none border-t-4 border-black/5">
            <CardHeader className="items-center">
              {artist.image && (
                <div className="relative w-32 h-32 sm:w-40 sm:h-40 rounded-full overflow-hidden border-4 border-chart-1 mb-4 bg-black">
                  <Image
              ...
              <CardTitle className="text-center text-2xl font-headline uppercase italic">{artist.name}</CardTitle>
              <CardDescription className="text-center text-chart-1 font-bold uppercase tracking-widest text-[10px]">{artist.role}</CardDescription>
            </CardHeader>
            <CardContent className="text-center text-muted-foreground flex-grow px-8 pb-8">
              <p className="leading-relaxed">{artist.description}</p>
            </CardContent>
          </Card>
        ))}
      </div>
